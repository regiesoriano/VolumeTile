...
- Removed the 60 ms delay before showing the volume panel.
- Keeps the foreground service warm for 2.5 seconds after a tap, so quick repeated taps avoid another foreground-service cold start.
- Still uses the NoDisplay Activity path to collapse Quick Settings without a visible app window.
